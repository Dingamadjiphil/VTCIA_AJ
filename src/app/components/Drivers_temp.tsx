...
import { DriversRegistration } from "@/app/components/DriversRegistration";
...
export function Drivers() {
  ...
  // Mock data for fines
  const finesData = [
    { id: 1, amount: 135, date: "2026-01-10", type: "Excès de vitesse", status: "Payé" },
    { id: 2, amount: 68, date: "2025-12-28", type: "Stationnement interdit", status: "Payé" },
    { id: 3, amount: 90, date: "2025-12-15", type: "Feu rouge", status: "En attente" },
  ];
    ...
    <div className="p-8 max-w-[1920px] mx-auto">
      ...
      {activeTab === "liste" && (
        <>
          ...
          {/* Drivers List */}
          <div className="bg-white/5 border border-white/10 p-6">
            <h2 className="text-xl font-bold text-white mb-6">LISTE DES CHAUFFEURS</h2>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 bg-white flex items-center justify-center">
                        <span className="text-black font-bold">{driver.name.charAt(0)}</span>
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-6">
                      <div className="text-right">
                        ...
                        {driver.currentVehicle ? (
                          <p className="text-white font-medium">{driver.currentVehicle}</p>
                        ) : (
            ...
            <div className="p-6 border-b border-white/10 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-white flex items-center justify-center">
                  <span className="text-black font-bold text-xl">{selectedDriver.name.charAt(0)}</span>
                </div>
                <div>
                  <h2 className="text-2xl font-bold text-white">{selectedDriver.name}</h2>
                  <p className="text-white/60 text-sm mt-1">Liste des paiements et statuts</p>
                </div>
              </div>
              <button
              ...
              {paymentsTab === "recettes" && (
                <>
                  ...
                  <div className="mb-4 flex gap-2">
                    ...
                    >
                      DATE {recettesSortBy === "date" && (recettesSortOrder === "asc" ? "↑" : "↓")}
                    </button>
                    <button
                    ...
                    >
                      MONTANT {recettesSortBy === "amount" && (recettesSortOrder === "asc" ? "↑" : "↓")}
                    </button>
                    <button
                    ...
                    >
                      STATUT {recettesSortBy === "status" && (recettesSortOrder === "asc" ? "↑" : "↓")}
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                    {sortedPayments.map((payment) => (
                      ...
                      >
                        <div>
                          <p className="text-white font-medium">Paiement du {payment.date}</p>
                          <p className={`text-lg font-bold mt-1 ${payment.status === "En attente" ? "text-red-500" : "text-white"}`}>
                            {payment.amount} F
                          </p>
                        </div>
                        <div className="flex items-center gap-4">
                  ...
                  <div className="mt-6 p-4 bg-white/5 border border-white/10">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <p className="text-xl font-bold text-red-500">
                        {payments.filter((p) => p.status === "En attente").reduce((sum, p) => sum + p.amount, 0)} F
                      </p>
                    </div>
                  </div>
                </>
              ...
              {paymentsTab === "epargne" && (
                <>
                  ...
                  <div className="mb-4 flex gap-2">
                    ...
                    >
                      DATE {epargneSortBy === "date" && (epargneSortOrder === "asc" ? "↑" : "↓")}
                    </button>
                    <button
                    ...
                    >
                      MONTANT {epargneSortBy === "amount" && (epargneSortOrder === "asc" ? "↑" : "↓")}
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                    {sortedSavings.map((saving) => (
                      ...
                      >
                        <div>
                          <p className="text-white font-medium">Versement du {saving.date}</p>
              ...
              {paymentsTab === "assurance" && (
                <>
                  ...
                  <div className="mb-4 flex gap-2">
                    ...
                    >
                      DATE {assuranceSortBy === "date" && (assuranceSortOrder === "asc" ? "↑" : "↓")}
                    </button>
                    <button
                    ...
                    >
                      MONTANT {assuranceSortBy === "amount" && (assuranceSortOrder === "asc" ? "↑" : "↓")}
                    </button>
                    <button
                    ...
                    >
                      TYPE {assuranceSortBy === "type" && (assuranceSortOrder === "asc" ? "↑" : "↓")}
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                    {sortedInsurance.map((insurance) => (
                      ...
                      >
                        <div>
                          <p className="text-white font-medium">{insurance.type}</p>
              ...
              {paymentsTab === "contraventions" && (
                <>
                  ...
                  <div className="mb-4 flex gap-2">
                    ...
                    >
                      DATE {finesSortBy === "date" && (finesSortOrder === "asc" ? "↑" : "↓")}
                    </button>
                    <button
                    ...
                    >
                      MONTANT {finesSortBy === "amount" && (finesSortOrder === "asc" ? "↑" : "↓")}
                    </button>
                    <button
                    ...
                    >
                      STATUT {finesSortBy === "status" && (finesSortOrder === "asc" ? "↑" : "↓")}
                    </button>
                    <button
                    ...
                    >
                      TYPE {finesSortBy === "type" && (finesSortOrder === "asc" ? "↑" : "↓")}
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                    {sortedFines.map((fine) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-4">
                          <p className={`text-lg font-bold ${fine.status === "En attente" ? "text-red-500" : "text-white"}`}>
                            {fine.amount} F
                          </p>
                          <Badge
                            className={
                  ...
                  <div className="mt-6 p-4 bg-white/5 border border-white/10">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <p className="text-white/60">EN ATTENTE</p>
                      <p className="text-xl font-bold text-red-500">
                        {finesData.filter((f) => f.status === "En attente").reduce((sum, f) => sum + f.amount, 0)} F
                      </p>
                    </div>
                  </div>
                </>
      ...
      {showDocumentsDialog && selectedDriver && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-[#0a0a0a] border border-white/10 w-full max-w-3xl max-h-[80vh] overflow-hidden flex flex-col">
            ...
            <div className="p-6 border-b border-white/10 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-white">DOCUMENTS - {selectedDriver.name}</h2>
                <p className="text-white/60 text-sm mt-1">Documents fournis par le chauffeur</p>
              </div>
              <button
                onClick={closeAllDialogs}
                className="w-10 h-10 bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/10 transition-all"
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-white/40 text-xs mt-1">Uploadé le {doc.uploadDate}</p>
                        </div>
                      </div>
      ...
      {showVehicleDialog && selectedDriver && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-[#0a0a0a] border border-white/10 w-full max-w-2xl">
            ...
            <div className="p-6 border-b border-white/10 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-white">GESTION VÉHICULE - {selectedDriver.name}</h2>
                <p className="text-white/60 text-sm mt-1">Réassigner un véhicule et définir le plafond</p>
              </div>
              <button
                onClick={closeAllDialogs}
                className="w-10 h-10 bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/10 transition-all"
            ...
            <div className="p-6">
              ...
              {/* Select New Vehicle */}
              <div className="mb-6">
                <label className="text-white/60 text-sm mb-2 block">NOUVEAU VÉHICULE</label>
                <select
                ...
                >
                  ...
                  {availableVehicles.map((vehicle) => (
                    <option key={vehicle} value={vehicle} className="bg-[#0a0a0a]">
                      {vehicle}
                    </option>
                  ))}
                </select>
              ...
              {/* Daily Limit */}
              <div className="mb-6">
                <label className="text-white/60 text-sm mb-2 block">PLAFOND RECETTE JOURNALIÈRE (F CFA)</label>
                <input
                  type="number"
                  value={vehicleForm.dailyLimit}
                  onChange={(e) => setVehicleForm({ ...vehicleForm, dailyLimit: Number(e.target.value) })}
                  className="w-full bg-white/5 border border-white/10 text-white p-3 hover:border-white/20 focus:border-white/30 outline-none transition-all"
                  placeholder="500"
                />
              </div>
      ...
      {showEditDialog && selectedDriver && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-[#0a0a0a] border border-white/10 w-full max-w-2xl">
            ...
            <div className="p-6 border-b border-white/10 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-white">MODIFIER - {selectedDriver.name}</h2>
                <p className="text-white/60 text-sm mt-1">Modifier le statut, téléphone et plafond</p>
              </div>
              <button
                onClick={closeAllDialogs}
                className="w-10 h-10 bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/10 transition-all"
            ...
            <div className="p-6">
              ...
              <div className="mb-6">
                ...
                <select
                  ...
                  onChange={(e) => setEditForm({ ...editForm, status: e.target.value as "Actif" | "Inactif" | "Licencié" })}
                  className="w-full bg-white/5 border border-white/10 text-white p-3 hover:border-white/20 focus:border-white/30 outline-none transition-all"
                >
                  <option value="Actif" className="bg-[#0a0a0a]">Actif</option>
                  <option value="Inactif" className="bg-[#0a0a0a]">Inactif</option>
                  <option value="Licencié" className="bg-[#0a0a0a]">Licencié</option>
                </select>
              </div>

              {/* Group */}
              <div className="mb-6">
                <label className="text-white/60 text-sm mb-2 block">GROUPE</label>
                <select
                  value={editForm.group}
                  onChange={(e) => setEditForm({ ...editForm, group: e.target.value as "A" | "B" | "C" | "D" | "E" | "F" })}
                  className="w-full bg-white/5 border border-white/10 text-white p-3 hover:border-white/20 focus:border-white/30 outline-none transition-all"
                >
                  <option value="A" className="bg-[#0a0a0a]">A</option>
                  <option value="B" className="bg-[#0a0a0a]">B</option>
                  <option value="C" className="bg-[#0a0a0a]">C</option>
                  <option value="D" className="bg-[#0a0a0a]">D</option>
                  <option value="E" className="bg-[#0a0a0a]">E</option>
                  <option value="F" className="bg-[#0a0a0a]">F</option>
                </select>
              </div>
      ...
      {showNewDriverDialog && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-[#0a0a0a] border border-white/10 w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
            ...
            <div className="p-6 border-b border-white/10 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-white">NOUVEAU CHAUFFEUR</h2>
                <p className="text-white/60 text-sm mt-1">Créer un nouveau profil chauffeur</p>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto flex-1">
              {/* Name */}
              <div className="mb-6">
                <label className="text-white/60 text-sm mb-2 block">NOM ET PRÉNOMS</label>
                <input
                  type="text"
              ...
              <div className="mb-6">
                ...
                <div className="relative">
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => setNewDriverForm({ ...newDriverForm, photo: e.target.files?.[0] || null })}
                ...
                {newDriverForm.photo && (
                  <p className="text-white/60 text-sm mt-2">Fichier sélectionné : {newDriverForm.photo.name}</p>
                )}
              </div>

              {/* CNI */}
              <div className="mb-6">
                <label className="text-white/60 text-sm mb-2 block">CARTE NATIONALE D'IDENTITÉ (CNI)</label>
                <div className="relative">
                  <input
                    type="file"
                    accept=".pdf,.jpg,.jpeg,.png"
                    onChange={(e) => setNewDriverForm({ ...newDriverForm, cni: e.target.files?.[0] || null })}
                    className="w-full bg-white/5 border border-white/10 text-white p-3 hover:border-white/20 focus:border-white/30 outline-none transition-all file:mr-4 file:py-2 file:px-4 file:border-0 file:bg-white/10 file:text-white file:cursor-pointer hover:file:bg-white/20"
                  />
                </div>
                {newDriverForm.cni && (
                  <p className="text-white/60 text-sm mt-2">Fichier sélectionné : {newDriverForm.cni.name}</p>
                )}
              </div>

              {/* Permis */}
              <div className="mb-6">
                <label className="text-white/60 text-sm mb-2 block">PERMIS DE CONDUIRE</label>
                <div className="relative">
                  <input
                    type="file"
                    accept=".pdf,.jpg,.jpeg,.png"
                    onChange={(e) => setNewDriverForm({ ...newDriverForm, permis: e.target.files?.[0] || null })}
                    className="w-full bg-white/5 border border-white/10 text-white p-3 hover:border-white/20 focus:border-white/30 outline-none transition-all file:mr-4 file:py-2 file:px-4 file:border-0 file:bg-white/10 file:text-white file:cursor-pointer hover:file:bg-white/20"
                  />
                </div>
                {newDriverForm.permis && (
                  <p className="text-white/60 text-sm mt-2">Fichier sélectionné : {newDriverForm.permis.name}</p>
                )}
              </div>

              {/* Certificat */}
              <div className="mb-6">
                <label className="text-white/60 text-sm mb-2 block">CERTIFICAT DE RÉSIDENCE</label>
                <div className="relative">
                  <input
                    type="file"
                    accept=".pdf,.jpg,.jpeg,.png"
                    onChange={(e) => setNewDriverForm({ ...newDriverForm, certificat: e.target.files?.[0] || null })}
                    className="w-full bg-white/5 border border-white/10 text-white p-3 hover:border-white/20 focus:border-white/30 outline-none transition-all file:mr-4 file:py-2 file:px-4 file:border-0 file:bg-white/10 file:text-white file:cursor-pointer hover:file:bg-white/20"
                  />
                </div>
                {newDriverForm.certificat && (
                  <p className="text-white/60 text-sm mt-2">Fichier sélectionné : {newDriverForm.certificat.name}</p>
                )}
              </div>
